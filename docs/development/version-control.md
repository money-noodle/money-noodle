...
The sole integration checkout is the worktree on symbolic `main`; authored work, merge commits, rebases, resets, conflict resolution, and history rewriting there are forbidden. Its normal state is a clean exact mirror of directly verified remote `main`; a clean ancestor is explicit fast-forward lag, not permission to update it. Only a separately authorized clean conflict-free fast-forward to the exact verified remote head may change local `main`. [`parallel-work.md`](parallel-work.md#integration-checkout-remote-lifecycle-and-integration-holds) owns the executable hook, read-only status, remote-lifecycle, exact bootstrap, scratch-worktree, and integration-hold contracts.
