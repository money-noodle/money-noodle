...
Keep work single-purpose and commits reviewable with imperative subjects. Initial post-#42 claims use the remote reference represented by the reserved derived branch `claim-v1/issue-<N>` and full ref `refs/heads/claim-v1/issue-<N>`; the pure coordination-schema boundary derives and parses that identity, while the dedicated claim module validates the complete write and creates that ref at the verified current `main` commit before writing ownership. The exact #42 bootstrap branch `arch/remote-reference-claim-primitive` is the sole pre-activation exception. A valid exact derived ref may remain as visible preserved non-ownership evidence after its schema-v2 issue becomes principal-owned `blocked` with all agent ownership unclaimed or reaches a coherent terminal state; preservation grants no current claim, lifecycle monitoring, release, deletion, takeover, cleanup, or ref-mutation authority. A derived ref mapped to parked `proposed` or `ready` work remains a fail-closed orphan. Reserved refs are never adopted, released, renamed, repointed, deleted, or cleaned automatically. Incorporate current `main` before integration. Do not commit directly to `main`; merge only after required checks and the applicable review or temporary-exception policy. Protect `main` with required review and checks, no direct push or history rewriting, and the deployment controls in [`../operations/delivery.md`](../operations/delivery.md).
