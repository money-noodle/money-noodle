...
The exception may waive **only** unavailable independent pull-request approval. Every other integration control remains mandatory:

1. Integration still occurs through a pull request; direct push to `main`, force push, history rewriting, protection weakening, and bypass of conversation resolution, stale-review dismissal, or last-push approval remain forbidden.
2. Immediately before the exception merge, all four required checks—`affected projects and repository gates`, `secret scan`, `container platform-api`, and `container web`—must have passed for the pull request's exact current head commit. Stale, missing, pending, cancelled, skipped-required, neutral-required, or failed evidence cannot qualify. Any head change invalidates earlier evidence.
3. The maintainer records durable public evidence identifying the pull request, its exact qualifying head commit, the resulting `main` commit, the specific reason an independent eligible reviewer was unavailable, and every required check's name, successful conclusion, and run reference for that exact head.
4. The merge must preserve every security, tenant, audit, funded-authority, delivery, and production-approval boundary. A green check or evidence record proves a condition only; it never grants authority to an agent, workload identity, or automation.
