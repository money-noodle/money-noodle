...
The integration checkout is the sole worktree on symbolic `main`. It is a read-only principal-operated mirror for normal coordination: its index and worktree, including untracked files, are clean; no merge, rebase, cherry-pick, revert, bisect, or sequencer operation is in progress; it has no local-only commit; and its head is either identical to directly read GitHub `refs/heads/main` (`mirrored`) or is its ancestor (`fast-forward-lag`). Status distinguishes `mirrored`, `fast-forward-lag`, `local-ahead`, `dirty-or-in-progress`, `divergence`, and `unavailable`. Only a separately authorized, clean, conflict-free fast-forward to the exact directly verified remote head may update local `main`. Every other state fails closed for integration and is preserved for a principal decision; status never resets, repairs, cleans, fetches, or changes a ref.

Committed `.githooks/pre-commit` and `.githooks/pre-merge-commit` refuse normal porcelain authored commits and non-fast-forward merge commits on `main`. A fast-forward creates no merge commit and remains possible only under the separate authority above; topic-branch commits and authorized scratch-branch merge reproduction remain possible. Hooks are local, bypassable defense in depth and cannot intercept resets, plumbing, or every mutation. They are inert until the principal separately authorizes repository-local `core.hooksPath: .githooks`. Read-only status reports every observed hooksPath entry, its effective value, scope and origin, both committed hook modes, and the integration-checkout classification; it never configures or repairs hooks.
...
Immediately before branch and worktree creation, directly read GitHub `refs/heads/claim-v1/issue-<N>` and require five SHA surfaces to agree: the registered checkpoint, direct GitHub claim ref, local remote-tracking ref, local branch, and dedicated worktree HEAD. Require upstream `origin/claim-v1/issue-<N>`, configured remote `origin`, configured merge ref `refs/heads/claim-v1/issue-<N>`, exactly one clean unlocked non-prunable worktree for the branch, and zero push. Registry evidence records the portable host label, never the dedicated local path. Any existing local branch, remote-tracking mismatch, worktree collision, partial command result, or ambiguous evidence is preserved for principal review; bootstrap never adopts, resets, repoints, force-updates, removes, or cleans it.

For every schema-v2 active or review claim on a derived ref, status compares exact `Checkpoint-Commit` with the exact directly read remote branch head and reports exactly one lifecycle: `matched`, `local-ahead` (ordinary unpublished checkpoint lag), `remote-ahead` (stale checkpoint), `missing-branch`, `divergence`, or `unavailable`. Missing, divergent, and unavailable evidence fails closed and becomes a maintainer question. Classification never grants adoption, publication, takeover, release, deletion, cleanup, or ref mutation.
...
The GitHub author equals `Integration-Hold-Principal`. An acquisition has matching acquired/event timestamps and outcome `unclaimed`. A release has outcome `integrated` or `aborted`, repeats every acquisition identity field byte-for-byte, and has an event timestamp no earlier than acquisition. Each acquisition and release requires separate principal authorization. Zero unmatched valid acquisitions and no malformed evidence is `clear`; exactly one is `held`; malformed evidence, an orphan or duplicate event, identity disagreement, or multiple unmatched acquisitions is `question`. Holds have no expiry, transfer, takeover, cleanup, or automatic release. They are advisory because status cannot prove an agent did not operate a shared credential.
