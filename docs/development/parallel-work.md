...
- `registry`: `repository`, required/missing `labels`, `plans`, `workItems`, paginated `remoteClaims`, `pullRequests`, and flattened `maintainerQuestions` when `coordinationKnown` is true; it is `null` when required evidence cannot be retrieved or validated; each plan includes its `integrationHold` evidence and `clear`, `held`, or `question` status, while each reserved ref includes its reconciliation disposition and whether it represents current ownership or lifecycle monitoring;
