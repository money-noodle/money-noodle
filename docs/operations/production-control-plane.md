...
`repositoryIdentity` is the fixed public repository identity binding established at bootstrap and independently compared to the immutable host repository identity (native numeric IDs remain private). `originalApprovalIdentity` is the immutable consent record identity: for a merge, `{kind: "merge", repositoryIdentity, pullRequestNumber, approvedHeadSHA, mergeSHA}` binds the exact reviewed head and resulting protected-main commit; for explicit consent, `{kind: "explicit", approvalRef, approvalBodyDigest}` uses the nonrecursive projection defined below. An explicit approvalRef is exactly `{issueNumber, commentId}` in repositoryIdentity; verify its association and authorizing principal against the original host record, not body text alone. Edits invalidate the record; they never renew it. Epoch and workflow/run/attempt identity are excluded from the key. Operation, configuration or target changes cannot recreate a spent key; changed consent needs a genuinely new principal decision, and cannot override unresolved effects. The same original consent cannot be relabeled with a new approval identity by a retry.
...
For explicit consent, derive the identities in this order (field omission is not null substitution):

1. `consentBody` is the exact completed-consent field set listed above **minus only originalApprovalIdentity**. It retains approvalRef, principal, requester, repositoryIdentity, original validity and all approved effect/verification/recovery bindings. No epoch, executor owner/run, grantKey, approvalBodyDigest or consentDigest is a top-level consent field.
2. `approvalBodyDigest = SHA256(JCS(consentBody))`. Set originalApprovalIdentity to `{kind: "explicit", approvalRef: consentBody.approvalRef, approvalBodyDigest}`. This body identity digest never hashes itself or the completed identity.
3. `consent = {...consentBody, originalApprovalIdentity}`. `consentDigest = SHA256(JCS(consent))` identifies this completed envelope in journal events; consentDigest is stored **outside** consent, never inside originalApprovalIdentity. It is distinct from approvalBodyDigest. Compute grantKey using the formula above after identity construction.

For merge consent, the fixed host identity tuple likewise contains no consentDigest; journal events hash the completed consent envelope identically. Component input/plan/policy/control-dependency/verification/recovery digests are computed from their own independently identified inputs before assembling consent; none may recursively include the consent envelope, its identity digest or grantKey. Source publication separately omits requestDigest from its own preimage and hashes only change entries for changesDigest. Git events record only the already-known parent commit; neither acknowledgment nor its snapshot records its own containing commit hash. These are serialization rules, not new approval authority: changed body, identity, principal or issue/comment cannot be repaired by recomputing hashes under old consent.

#### Canonical explicit-consent example

The following complete canonical envelope uses synthetic public labels, issue/comment numbers and repeated-hex input identifiers only; it grants nothing. Hash preimages are precisely the UTF-8 JSON bytes without a trailing newline. The source-contract test independently reconstructs consentBody, identity, completed-envelope digest and grantKey, including a reordered-object input and changed-principal/approval/expiry negative cases.

```json
{"approvalRef":{"commentId":2,"issueNumber":1},"artifactVector":[],"catalog":"money-noodle.production-operations","controlDependencyDigest":"3333333333333333333333333333333333333333333333333333333333333333","controlSourceSHA":"2222222222222222222222222222222222222222","effectBounds":{"set-display-mode":1},"environment":"synthetic-production","executorClass":"configurator","expiresAt":"2026-09-13T01:00:00Z","inputDigest":"5555555555555555555555555555555555555555555555555555555555555555","issuedAt":"2026-09-13T00:00:00Z","notBefore":"2026-09-13T00:00:00Z","operation":"configuration.change","originalApprovalIdentity":{"approvalBodyDigest":"7a4d3189090b981d1480d792b10550693a268cbbcfc6914c1528039db28ef36d","approvalRef":{"commentId":2,"issueNumber":1},"kind":"explicit"},"permissionSlot":"configuration-change","planDigest":null,"policyDigest":"1111111111111111111111111111111111111111111111111111111111111111","principal":"synthetic-principal","readBounds":{"maxBytes":4096,"maxPages":1,"timeoutSeconds":10,"validUntil":"2026-09-13T01:00:00Z"},"reasonRef":"synthetic-change-1","recovery":null,"recoveryContractDigest":"7777777777777777777777777777777777777777777777777777777777777777","repositoryIdentity":"synthetic-repository","requestId":"synthetic-request-1","requester":"synthetic-agent","sourceSHA":"4444444444444444444444444444444444444444","targetVector":[{"actionCounts":{"set-display-mode":1},"configurationVersion":"config-1","expectedSafeVersions":{"displayMode":"compact"},"intendedSafeVersions":{"displayMode":"expanded"},"logicalIncarnation":"synthetic-api-1"}],"transportPhases":[{"actionId":"set-display-mode","maxSubmissions":1,"phaseId":"set-config","targetIds":["synthetic-api-1"]}],"verificationContractDigest":"6666666666666666666666666666666666666666666666666666666666666666","verifierClass":"config-behavior-reader","version":2}
```

| Derived value | Expected SHA-256 |
| --- | --- |
| approvalBodyDigest | `7a4d3189090b981d1480d792b10550693a268cbbcfc6914c1528039db28ef36d` |
| consentDigest | `9bbdb761c945b1a17c2b3827de17e3c2d46ebac3274e95c7ab674f238c2daef8` |
| grantKey | `26d5b8ff57e472ba6d21019c7f4bec60d3bab6383d88517450f0f2a8806b6d9e` |
...
| --- | --- | --- |
| `control/current.json` | `epoch`, `sequence`, `latestEventId`, `activeRequestId`, `pendingIntent`, `witness`, `lastAcknowledgment` | One global active M1 provider request. Nullable pendingIntent is `{eventId, sequence}`. Nullable witness is the last confirmed `{epoch, sequence, eventId, journalCommit, commentId}`. lastAcknowledgment is null only before the first confirmed acknowledgment, otherwise `{eventId, sequence}` indexing that immutable event, not its containing commit. |
...
1. Read journal head `H`; fetch indexed snapshots and required event with `GET /repos/{repo}/contents/{path}?ref={H}`. Read the designated witness issue-body pointer and comment directly by ID. Validate coherent snapshots and the current latest event at H. Resolve control.lastAcknowledgment directly to `events/{eventId}.json?ref=H`, then its witnessAck.eventId directly to the intent at H. Require eventType witness-ack and acknowledgment.sequence = intent.sequence + 1 = lastAcknowledgment.sequence; match causationId/previousEventId to the intent and parentCommit to witnessAck.journalCommit. Require equality of witnessAck, control.witness and the current issue pointer; compare their shared epoch/sequence/eventId/journalCommit fields with the comment read by commentId, and match that comment's grantKey/executorOwner to the intent. Current latest event must carry the same lastAcknowledgment index, even when latestEventId names later evidence. This is a bounded pair of indexed event reads, not an ancestry/history scan; missing or inconsistent links block. Also validate epoch, consent key, expiry, finite bounds, expected state and no outstanding competing owner. Allow at most one global active M1 provider request, including publication; source publication has its separate claim serialization.
...
4. Append `witness-ack` event at sequence `n+1`, sole parent `C`, atomically clearing pendingIntent, setting control.witness to the confirmed pointer and setting control.lastAcknowledgment and the acknowledgment event's lastAcknowledgment to `{eventId: acknowledgmentEventId, sequence: n+1}`. This event-ID self-reference is not a commit hash. `witnessAck` contains that pointer; its causationId names the intent. This acknowledgment changes no consent, spent state or executor owner. **Do not witness the acknowledgment** or advance the issue pointer to its commit. For mutation, require head to be this exact acknowledgment, whose parent/event and witness comment/pointer match the intent. The next intent must have the current coherent head as its sole parent, not necessarily the previous acknowledgment commit. Submission/observation/verification events may have advanced that head; preserve all of them and obtain a fresh witness for the new intent. Thus latest event may be acknowledgment while pointer names the preceding intent, or later evidence while lastAcknowledgment still indexes that acknowledgment, without recursion.
5. Only after journal plus witness confirmation may the original owner exchange a mutation token. Repeat durable intent and corroboration before each external phase. Record submission/observation/verification as append-only events with atomic snapshot updates; each preserves control.witness and lastAcknowledgment until a new acknowledgment atomically replaces both. Evidence-only events never authorize a call. The new intent also preserves the prior pair while pending; a newly advanced witness pointer without its exact acknowledgment remains pending/blocked for mutation. Before another call a new intent/ack relation must be established. Completion releases the global slot only after quiescence and old-owner exclusion; an abandoned or unknown call does not release it by timeout.

#### Evidence-preserving sequence fixture

Commit letters below are synthetic labels, not literal Git hashes. Each event ID equals its lowercase commit label for this fixture; each append uses the current coherent head as sole parent. C starts from already coherent H; C and its pending snapshot retain H's prior acknowledgment/witness (omitted from this projection). From A onward, the table gives the complete acknowledgment-index and witness-commit projection. At V, the fixed reads are current control/request/at-most-two-target snapshots, latest event v, acknowledgment a and intent c: at most seven snapshot/event documents, plus direct witness pointer/comment and necessary commit metadata within the existing request/byte/time budgets. No traversal of S/O is needed to resolve the acknowledgment; their Git history and immutable events remain retained.

| Commit | Sole parent | Event type | Sequence | lastAcknowledgment.eventId | lastAcknowledgment.sequence | Witness journalCommit |
| --- | --- | --- | --- | --- | --- | --- |
| A | C | witness-ack | 11 | a | 11 | C |
| S | A | submission | 12 | a | 11 | C |
| O | S | observation | 13 | a | 11 | C |
| V | O | verification | 14 | a | 11 | C |
| I | V | intent | 15 | a | 11 | C |
| B | I | witness-ack | 16 | b | 16 | I |

C is intent sequence 10 and A's witnessAck names c/C; B's witnessAck names i/I. Read C/I by those indexed event IDs, not by searching for commits containing them. I must parent V, not A: parenting A would omit S/O/V and fail ordinary fast-forward publication. Reject altered parents, broken acknowledgment indexes, changed owner/consent/spent slots, or any new call at S/O/V/I. Only the original owner at a freshly confirmed A or B can submit its corresponding phase, subject to unchanged expiry, expected state and uncertainty checks. V's verification does not bypass quiescence/old-owner-exclusion requirements or mint new consent. The acknowledgment is never witnessed and never contains its own commit hash.
...
| witness-ack | C → A → S → O → V → I → B, with pointer C and indexed acknowledgment a through V/I | I must parent current head V, not A; retain S/O/V, read a/c by fixed indexes, corroborate I with B without witnessing B |
