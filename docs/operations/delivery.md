...
`main` is production. After v2 cutover, merging to `main` automatically invokes production delivery. GitHub Actions may orchestrate provider-native automation, but every stage and final verification reports to the commit. Merge is deployment authorization; agents do not merge without explicit instruction.

During the rebuild, `v2` receives integration CI without requiring a standing non-production deployment. Production CD, branch protection, and cutover validation must exist before v2 reaches `main`.
...
Protect `main` with one required reviewer and all required checks, no direct push/history rewriting, deployment concurrency, and serialization for unsafe infrastructure/schema operations.
