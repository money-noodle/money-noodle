...
- Use Google Cloud Storage as the platform's single object store, including the initial historical/analytical store, through a portable object-storage adapter. See proposed [`ADR-0008`](decisions/ADR-0008-single-object-store.md).
- Do not add a second provider/region. Keep historical/analytical buckets separate from infrastructure-state buckets, and keep object manifests, checksums, versioning, inventory, and restore procedures replication-ready so another failure domain can be added later without changing domain code.
