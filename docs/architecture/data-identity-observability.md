...
The proposed production-operation evidence contract extends that envelope with the catalog/operation version, human authorization grant, requester/executor/verifier separation, safe input/plan/expected-state digests, lease or fencing generation, intended-recorded-observed comparison, recovery action, and terminal verification state; see [`../operations/production-control-plane.md`](../operations/production-control-plane.md). It records no secret payload, raw provider state, account/billing identifier, customer data, or private recovery material. Public coordination receives only a redacted opaque evidence reference. If durable audit is unavailable, a consequential mutation does not start; an uncertain effect freezes for reconciliation rather than continuing unaudited.
