...
- Use Scaleway's S3-compatible object storage as the initial historical/analytical store through a portable object-storage adapter. This accepted direction remains current unless and until proposed [`ADR-0008`](decisions/ADR-0008-single-object-store.md) is separately accepted and becomes Working; only then would ADR-0008 replace it with Google Cloud Storage.
- Do not add another historical/analytical object-storage provider or region under the current direction. Keep object manifests, checksums, versioning, inventory, and restore procedures replication-ready so another failure domain can be added later without changing domain code.
...
- **First-slice exception, accepted 2026-09-15.** Only one of those three classes turned out to be configurable on the selected backend. Application and debug logs are set explicitly to 14 days; trace retention is Google's documented 30 days and OTLP metric retention is its documented 24 months with progressive downsampling, both recorded as provider behaviour rather than as configured TTLs. See the [ADR-0007 amendment](decisions/ADR-0007-first-telemetry-backend.md#accepted-amendment-2026-09-15-attainable-retention). The exception is scoped to the financially inert, allowlisted-metadata first slice and adds no second backend, archive or deletion job; audit and accounting retention are untouched. Desired configuration is not observed retention.
...
The production-operation evidence contract decided in Working [`ADR-0010`](decisions/ADR-0010-agent-operated-production-control-plane.md) extends that envelope with the catalog/operation version, human authorization grant, requester/executor/verifier separation, safe input/plan/expected-state digests, lease or fencing generation, intended-recorded-observed comparison, recovery action, and terminal verification state; see [`../operations/production-control-plane.md`](../operations/production-control-plane.md). No audit store implementing it exists yet, so this constrains the implementation rather than describing a running system. It records no secret payload, raw provider state, account/billing identifier, customer data, or private recovery material. Public coordination receives only a redacted opaque evidence reference. If durable audit is unavailable, a consequential mutation does not start; an uncertain effect freezes for reconciliation rather than continuing unaudited.
...
In the first slice this is enforced rather than assumed. Each service's telemetry adapter allowlists span names, attribute keys and event names before buffering and again before serialization, replaces an allowlisted field whose *value* matches a sensitive shape, drops any span event that is not allowlisted (an exception event's attributes are its message and stack), exports a status code with no provider text, and refuses correlation identifiers as metric labels so cardinality stays bounded. Route templates are exported, never raw URLs or query strings. The negative tests assert these properties over the serialized OTLP payload, not only over hand-authored spans.
