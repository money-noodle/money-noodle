...
1. the platform API serves the administrative view without making any provider billing, deployment, or infrastructure call on a request path, verified by a negative test that fails if such a call is attempted;
2. no provider credential appears in any response, any client bundle, or any browser-reachable path;
3. the ingestion unit runs as a short-lived scheduled invocation under an identity distinct from the deployer and from every service runtime identity, and holds no write or administrative billing permission;
4. the budget's ceiling and thresholds reaching the surface are the values the budget enforces, proven by changing the ceiling in one place and observing the surface follow;
5. every rendered figure carries a source and an as-of time, and the provider data time is distinguishable from the ingestion run time;
6. a suppressed, failed, or partial ingestion run renders explicitly unknown or stale and never `$0` and never healthy, proven by a deliberate negative test;
7. a principal with platform operations and without billing/ownership sees deployment and health state and no spend figure, and the reverse holds, with authorization-denied absence visibly distinct from data-quality unknown;
8. no code path in the surface can scale, cap, disable, or otherwise act on any service, verified by inspection of the deployed unit's granted permissions;
9. derived free-tier headroom is labeled as derived, cites the limit's source and as-of date, and is never presented with the same weight as reported spend;
10. the read model validates against its declared schema version and its checksum, and a corrupt or incompatible document is detected and rendered unknown rather than parsed optimistically;
11. no account identifier, billing-account identifier, project number, or billing datum appears in Git, in an issue, or in any rendered response;
12. the first dated remote spend figure is recorded with as-of time, source, exclusions, and largest validity threat before any figure on this surface is described as measured rather than estimated.

## Revisit when

- the first dated remote baseline replaces the U1, U4, and U7 estimates with measured spend, which is the evidence this surface was sequenced behind;
- identity and tenancy exist, which makes per-tenant attribution answerable and turns the deferred label-and-allocation path into real work;
- the number of deployable projects or the size of the bill makes a platform-wide total uninformative, which is the trigger for the deferred BigQuery export;
- the telemetry backend changes, since ingestion volume is the cost line most likely to move and `ADR-0007` selected its backend for reversibility;
- the accepted `$30` ceiling changes, since the surface reads it rather than restating it and any second source of truth would be a defect;
- a spend figure is ever needed as a durable accounting or audit record, which requires its own separate design and must not reuse this one;
- an automated cost-control action is genuinely wanted, which requires a separately accepted safety design and cannot be added to this surface incrementally.
