...
- **2026-09-08** — The coordination protocol keeps the atomic remote-reference claim, declared scope, the dependency graph, and a read-only status board, and drops the scope gates, evidence headers, reconciliation machinery, and schema duality built around them; the machinery had reached an 11:1 process-to-product ratio and its own status tool outgrew the subprocess buffer that every gate read (#110). See the correction in [`ADR-0011`](ADR-0011-agent-coordination-and-isolation-protocol.md#correction-2026-09-08-the-protocol-was-simplified).
