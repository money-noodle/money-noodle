...
GitHub Issues are the shared registry, and remote Git references provide the claim primitive. A claim is created only at `refs/heads/claim-v1/issue-<N>`; existing refs in that namespace remain current reservations where applicable and are never renamed or bulk-migrated. Only confirmed successful ref creation establishes the reservation for the execution agent explicitly named in that initial creation, whether that agent or its authorized owning supervisor makes the request. The [delegation contract](../../development/parallel-work.md#delegation-contract) governs supervisor preallocation without transfer, adoption, or a second claimant/writer. A failed or ambiguous response does not establish ownership; issue bookkeeping remains a separate, non-atomic host update and partial results are preserved for explicit resolution.
...
A delegated assignment remains a bounded child, never a root supervisor or human-directed debugger. Independent sessions infer execution or planning supervision from a clear principal request and clarify material ambiguity before consequential work. Only a direct human request initiates and drives a debugger; it works directly with that human and may vary ordinary diagnosis ceremony, but cannot be launched, delegated, or impersonated by supervisors and receives no additional authority. The owning role detail remains in [`parallel-work.md`](../../development/parallel-work.md#session-roles). Its [delegation contract](../../development/parallel-work.md#delegation-contract) separates native lifecycle from claim accountability and owns bounded briefs, returns, capability ceilings, and recovery; these are reviewed-source requirements, not runtime enforcement or deployment evidence.

Planning supervisors share a parent plan and serialize edits to the same plan section. They may make only small, isolated in-scope edits on a dedicated topic branch and worktree; substantial or parallel work is delegated. Execution workers use their own claimed branch and worktree at the canonical primary checkout's ignored `.worktrees/` location. Checkpoints are short and state the state, commit, next action, and blockers.
