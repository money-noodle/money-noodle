...
| [`ADR-0010`](ADR-0010-agent-operated-production-control-plane.md) | Working | Decision and catalog only; nothing implemented or applied | M1 catalog v2, bounded Git journal/witness custody and sole-principal release target; broader secret/operation design retained |
| [`ADR-0011`](ADR-0011-agent-coordination-and-isolation-protocol.md) | Working | Simplified claim and board tooling locally validated; not deployment | Cross-harness coordination, claim isolation, integration safety, and durable coordination |

The Evidence column is the point of this table. ADR-0001 through ADR-0007, ADR-0010, and ADR-0011 are **Working**: decided enough to build on, but not production-proven. ADR-0001 through ADR-0003 are exercised by repository checks, ADR-0004 through ADR-0007 have never been applied to a provider, ADR-0010 is a boundary decision and operation catalog with no implemented control plane behind it, and ADR-0011 has simplified claim and read-only board tooling exercised by local repository checks, not deployed validation. ADR-0008 and ADR-0009 are **Proposed**, not Working. The lack of a production deployment prevents any record from becoming Settled; it does not promote a proposal to Working.

ADR-0010 was proposed on 2026-08-30 under GitHub work item #20 and became Working on 2026-09-07 when the maintainer accepted it. Its operational-read boundary may consume the administrative read model if ADR-0009 is later accepted, but accepting ADR-0010 neither accepts nor promotes ADR-0008 or ADR-0009, and ADR-0010 does not depend on them. Working status settles the authority and boundary shape and authorizes implementation through reviewed code and pipeline changes; it grants no provider or production authority. No M1 fixed publisher, operation journal/witness, bounded administrative job or payload-blind secret ingress exists, the versioned catalog in [`../../operations/production-control-plane.md`](../../operations/production-control-plane.md) is a design rather than a running system, and implementation and remote validation remain separate work that has not started. The principal accepted the M1 v2 refinement on 2026-09-13: restricted workload publication, separately consumed release slots, existing-repository sanitized journal/witness and bounded private-input reconstruction. The [delivery transition](../../operations/delivery.md#current-to-target-activation) requires separately authorized host, operation and bootstrap qualification; current environment safeguards remain unchanged and unsatisfied.
