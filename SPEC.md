...
> **Status:** Draft 0.37 · **Updated:** 2026-08-14
...
- Paper mode uses the same policy, sizing, reservation, settlement, and state-machine semantics as production live execution while retaining a completely separate bankroll, ledger, P&L, and report. It is not serialized to live's one-order execution path: each cycle may submit every paper-selected candidate that fits the same portfolio, correlation, bankroll, and provider-funding constraints.
...
- Live Kalshi maker entry uses managed v2 `post_only` GTC selected-side bids. YES entries map to Kalshi YES-book bids; NO entries map to signed YES-book asks while all limits, costs, fills, P&L, and UI remain selected-side denominated. Maker joins/improves the selected-side bid over 12 seconds, amends toward ask-minus-one-valid-tick without crossing the issuance cap, confirms remainder cancellation, accepts partial fills, and reconciles exact fees. In `adaptive` mode an authoritative zero-fill maker may open one sequence-local taker fallback; a partial fill, taker no-fill, rejection, working order, uncertain state, or completed second attempt never does.
- A separately versioned maker/taker policy evaluates execution style before submission. In `maker` mode every intent remains maker and records the shadow taker decision. In `adaptive` mode attempt 1 takes only when six gates clear: at least 15pp current net edge after taker fee, 10pp persistence-median edge, 65% quality, spread no wider than 2¢, at least 30 comparable accepted maker samples, and at least 2pp advantage over empirically captured maker edge. The 2¢ ceiling bounds the price concession for immediacy; it is not a maker-fill predictor, and the separate 10¢ spread ceiling rejects an entry entirely. If attempt 1 is maker and authoritatively receives zero fill, there is no fixed cooldown: two new qualifying snapshots strictly after maker completion, spanning at least 15 seconds, must re-establish fresh authority. Attempt 2 still requires the four absolute edge, median, quality, and 2¢ spread gates; the local maker miss replaces only the sample-count and comparative-advantage gates. It is one marketable IOC **limit**, never an uncapped market order. Immediate and fallback takers may accept at most 1.0¢ ask movement from issuance, capped again at the 97¢ entry ceiling; quantity and fees reserve against that worst price, while the submitted limit is only the refreshed ask. The fresh quote must re-clear the applicable edge, median, quality, 2¢ spread, and comparative gates. Movement beyond 1¢ or a fresh gate failure produces no submission; later movement can leave an accepted IOC unfilled. Either result ends that logical asset/side/window/generation sequence. Other sequences start adaptive attempt 1 anew. Pre-submit quote refusal, accepted IOC no-fill, and rested maker no-fill remain separately labeled. See `docs/adaptive-entry-fallback-design.md`.
...
| Differs | Why |
|---|---|
| Fill model — maker post-only vs simulated maker | Paper originally filled at the ask, then used a static dashboard bid and ask-touch rule. Neither represented live: live refreshes the exact contract, reprices every two seconds, and can fill from incoming sells without a sampled ask touch. *(Implemented 2026-08-14: independent paper management now uses live's shared pricing transitions, exact quote/depth, public aggressor trade prints, displayed queue-ahead depletion, issuance-sized quantity, and concurrent two-second management. Ask touch is telemetry only. Authoritative matched-live fills are retained as a separate overlay rather than replacing the independent simulation.)* |
