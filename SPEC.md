...
> **Status:** Draft 0.42 · **Updated:** 2026-08-20
...
- Positive-edge and prediction cards reserve stable minimum height. A signal leaving the qualified set keeps its grid slot through a short opacity/blur transition, and re-entry before expiry cancels departure and replaces the stale snapshot. Signed awaiting-confirmation signals are visible by default rather than hidden behind a collapsed count.
...
- [x] Add a signed, stateful, on-demand Kalshi selected-side ladder to positive-edge cards without connecting UI reads to execution telemetry; stabilize card/grid heights, default awaiting-confirmation signals visible, and fade departing signals in place.
