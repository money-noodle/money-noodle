...
- The active policy is binary buy policy v11. A calculation qualifies only when independent `P(side)` is at least 55%, `P(side) − side ask − venue fees` is at least 5 percentage points, estimate quality is at least 50%, and the executable selected-side ask is from 5¢ through 97¢ on at least one live venue enabled in Budget. `P(DOWN)=1−P(UP)` and uses no venue input. The symmetric owned-side floor rejects cheap contracts when the model itself still calls that selected side the underdog.
