...
- Live Kalshi entry defaults to managed v2 `post_only` GTC selected-side bids. YES entries map to Kalshi YES-book bids; NO entries map to signed YES-book asks while all limits, costs, fills, P&L, and UI remain selected-side denominated. Maker joins/improves the selected-side bid over 12 seconds, amends toward ask-minus-one-valid-tick without crossing the issuance cap, confirms remainder cancellation, accepts partial fills, and reconciles exact fees. A zero-fill/post-only race may receive one bounded fresh retry after 30 seconds, but never converts to taker as a stale fallback.
