...
describe('background reconciliation scheduling boundary', () => {
  it('is started by the persistent runtime and is not awaited by the collector', async () => {
    ...
    const collector = await readFile(path.join(root, 'src/lib/background-collector.ts'), 'utf8');
    const instrumentation = await readFile(path.join(root, 'src/instrumentation.node.ts'), 'utf8');
