...
describe('bounded reporting read paths', () => {
  it('polls one compact public performance resource for both homepage panels', () => {
    ...
    const hook = source('src/components/use-public-paper.ts');
    const automation = source('src/components/automation-status.tsx');
  ...
  it('keeps complete performance reports on the dialog-open path only', () => {
    const dashboard = source('src/components/dashboard.tsx');
    const dialog = source('src/components/performance-dialog.tsx');
  ...
  it('never turns an unavailable hosted paper budget into a zero ledger', () => {
    const execution = source('src/lib/paper-execution.ts');
    const route = source('src/app/api/paper-budget/route.ts');
  ...
  it('keeps the signed polling route free of forecast-history imports', () => {
    const route = source('src/app/api/performance/summary/route.ts');
  ...
  it('owns one funded-control poll and leaves terminal order detail on demand', () => {
    const dashboard = source('src/components/dashboard.tsx');
    const automation = source('src/components/automation-status.tsx');
    const dialog = source('src/components/trading-control-dialog.tsx');
  ...
  it('keeps fixed order readers compact while explicit attributed history hydrates on demand', () => {
    expect(source('src/app/api/performance/summary/route.ts')).toContain('includeArchivedEvidence: false');
    expect(source('src/app/api/trading/history/route.ts')).toContain('includeArchivedEvidence: true');
    expect(source('src/app/api/trading/allocations/route.ts')).toContain('includeArchivedEvidence: false');
    expect(source('src/lib/public-paper-performance.ts')).toContain("strategyId: EDGE_BINARY_BUY, includeArchivedEvidence: false");
    expect(source('src/app/api/performance/route.ts')).toContain('getExecutionOrders()');
    const risk = source('src/lib/live-risk-store.ts');
  ...
  it('routes analyses through v9 hydration and makes historical correction tools refuse compact rows', () => {
    const files = ['src/lib', 'src/app', 'scripts'].flatMap((directory) => readdirSync(path.join(repoRoot, directory), {
