    ...
    <DialogContent className="max-w-4xl p-0">
      ...
      <div className="max-h-[82vh] overflow-y-auto p-5">
        {loading && !data ? <div className="grid h-64 place-items-center"><Loader2 className="animate-spin text-muted-foreground"/></div> : <>
          ...
          {data?.regimeGate && data.regimeGate.phase !== 'disabled' && <div className={cn('mb-4 rounded-lg border p-3', data.regimeGate.phase === 'closed' ? 'border-amber-300/30 bg-amber-300/[.06]' : 'border-primary/20 bg-primary/[.03]')}>
            <div className="flex flex-wrap items-center justify-between gap-2"><p className={cn('flex items-center gap-2 text-xs font-semibold', data.regimeGate.phase === 'closed' ? 'text-amber-100' : 'text-primary')}><ShieldAlert className="size-4"/>Adaptive regime gate</p><Badge variant="outline" className="uppercase">{data.regimeGate.phase}</Badge></div>
