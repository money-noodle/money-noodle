      ...
      <div className="max-h-[82vh] overflow-y-auto p-5">
        {loading && !data ? <div className="grid h-64 place-items-center"><Loader2 className="animate-spin text-muted-foreground"/></div> : <>
            ...
            return <div className={cn('mb-4 flex items-start gap-3 rounded-lg border p-3', active && live ? 'border-live/40 bg-live/[.07]' : active ? 'border-primary/25 bg-primary/[.05]' : 'border-primary/15 bg-primary/[.035]')}>
              ...
              <div>
                ...
                <p className="mt-1 text-[10px] leading-relaxed text-muted-foreground">{active
                  ? `Armed on ${armed.join(' + ') || 'no venue'}. Each asset/side/window allows up to ${data?.maximumLiveEntryEpisodes ?? 1} bounded entry episode${(data?.maximumLiveEntryEpisodes ?? 1) === 1 ? '' : 's'}; after a maker miss, the next episode must requalify from fresh post-completion snapshots. Each episode reserves its current all-in cap, cancels any maker remainder, and releases unused principal and fee reserve. Limits: ${data?.portfolioConstraints?.maximumPositions ?? 3} concurrent, ${data?.portfolioConstraints?.maximumSameWindow ?? 2} per window, ${data?.portfolioConstraints?.maximumSameGroupPerWindow ?? 1} per correlation group, ${dollars(data?.live?.proposedStakeCents ?? 0)} next purchase cap, ${data?.liveRisk.maximumCurrentEpochDrawdownPercent.toFixed(1) ?? '25.0'}% current-budget drawdown stop, ${dollars(data?.liveRisk.maximumLifetimeLossCents ?? 50)} lifetime-loss stop.`
          ...
          {data?.regimeGate && data.regimeGate.phase !== 'disabled' && <div className={cn('mb-4 rounded-lg border p-3', data.regimeGate.phase === 'closed' ? 'border-warn/30 bg-warn/[.06]' : 'border-data/20 bg-data/[.03]')}>
            ...
            <p className="mt-2 font-mono text-[9px] text-muted-foreground">Resolved policy windows {data.regimeGate.resolvedWindows} · minimum {data.regimeGate.configured.minimumPolicyWindows} · effective {data.regimeGate.effectiveWindows.toFixed(1)} · weighted recent fee-aware return {data.regimeGate.weightedMeanEdge === null ? '—' : `${data.regimeGate.weightedMeanEdge >= 0 ? '+' : ''}${(data.regimeGate.weightedMeanEdge * 100).toFixed(1)}pp`} · estimated negative-return probability {data.regimeGate.negativeReturnConfidence === null ? '—' : `${(data.regimeGate.negativeReturnConfidence * 100).toFixed(1)}%`}</p>
