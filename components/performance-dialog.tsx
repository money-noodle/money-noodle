...
function MakerExecutionPanel({ report }: { report: MakerFillReport }) {
  return <div className="mt-3 space-y-3 rounded-lg border p-3">
    <div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-[10px] font-medium">Maker execution and adverse-selection funnel</p><p className="mt-0.5 text-[9px] text-muted-foreground">Observation only. Acceptance, queue fill, and settlement profitability are distinct events; none of these metrics alter live execution.</p></div><Badge variant="outline" className="font-mono">{report.submittedAttempts} submitted</Badge></div>
