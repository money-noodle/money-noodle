...
This policy applies to Pi. The flagship mandate belongs only to root or supervisor Pi sessions; execution children are explicitly exempt. The designated most-capable primary supervisor route remains `openai-codex/gpt-6-astra:high`. The principal explicitly authorizes `pi-claude-code-provider/fable:high` as its contingency route, not a new capability ranking. Main Pi uses GPT first, then Fable; the six native child roles use Fable first, then GPT. Both main routes default to high thinking. Do not silently change these approved routes or claim the fallback is more capable.

For every native child task, the supervisor selects an exact `provider/id:level` route based on ambiguity, reasoning difficulty, impact, context and tool requirements, verification strength, and remaining provider capacity. Mix Claude and GPT children deliberately. Prefer qualified Fable for substantive work while Claude has capacity, preserving GPT capacity for continued work and the supervisor. These are starting defaults, not hard enforcement or benchmark evidence:

| Task shape | Starting route | Approved alternative when capacity or task fit warrants |
| --- | --- | --- |
| Lookup/recon | `pi-claude-code-provider/fable:low` | `openai-codex/gpt-5.6-luna:low` |
| Mechanical edits/tests | `pi-claude-code-provider/fable:medium` | Explicitly selected `openai-codex/gpt-5.6-luna:medium` |
| Ordinary implementation, planning, research | `pi-claude-code-provider/fable:medium` | `openai-codex/gpt-6-astra:medium` (configured backup), or explicitly selected `openai-codex/gpt-5.6-terra:medium` |
| Difficult cross-cutting work/debugging | `pi-claude-code-provider/fable:high` | `openai-codex/gpt-6-astra:high` or `:xhigh` |
| Serious security, tenant, or financial review | `pi-claude-code-provider/fable:high` or `:max` | `openai-codex/gpt-6-astra:high` or `:max` |
