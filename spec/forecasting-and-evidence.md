...
<a id="req-forecast-profit-objective"></a>
...
<a id="req-forecast-target-measurement"></a>
...
<a id="req-forecast-edge-track-record"></a>
...
- `lib/policy-manifest.ts` owns the active policy identity and immutable version history. Under the required
  ...
  Changing any threshold requires a new policy version, matching history, evidence, and manual recorded decision.
...
<a id="req-forecast-resolution"></a>
...
<a id="req-forecast-performance"></a>
...
<a id="req-forecast-learning"></a>

#### Learning policy

- Stored outcomes and `calibration-replay-v1` issuance inputs feed explicit offline, local, versioned model
  evaluation and calibration. Replay inputs include raw reference/current price, clock, volatility, basis
  weight/probability, aggregate and individual slow-term log odds, production caps, and production probability, but
  never venue price. Eligible checkpoints are defined by independent-window counts; each run is immutable and
  feature-fingerprinted, reports exact/unavailable input coverage and replay error, and requires a separate manual
  production-promotion action. The funded worker does not schedule evaluator runs.
...
<a id="req-forecast-production-model"></a>
...
This remains a provisional production baseline rather than a validated calibrated model. Exact issuance-time replay
inputs are persisted, evaluator runs occur only through the guarded offline workflow, and any production promotion
is manual and versioned.

The dashboard marks an entry as a **positive-edge binary buy** under the issuance requirement above. It may open
UP/YES or DOWN/NO. Reduce-only exit recommendations remain a separate portfolio policy. Depth, queue priority, and
slippage cannot silently enrich the buy rule; the signed selected-side ladder is operator observation only. Ranking
and execution apply their separately owned maturity, freshness, portfolio, funding, timing, and reconciliation
requirements after qualification.

<a id="req-forecast-factor-evolution"></a>
...
<a id="req-forecast-calibration-evaluation"></a>
