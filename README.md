...
- Reports an observation-only maker funnel from submission through post-only race, acceptance, queue fill, and settlement. Resolved forecasts provide zero-spend counterfactual outcomes for accepted non-fills, allowing `P(UP | fill)` and net return to be compared with accepted orders that did not fill without altering the ledger.
