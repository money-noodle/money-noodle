...
describe('execution signal persistence', () => {
  it('blocks an otherwise persistent signal during the first minute', () => {
    let state: SignalPersistenceState | undefined;
    for (const seconds of [15, 30, 45]) state = advance(state, seconds);
    const result = evaluateSignalPersistence(state, Date.parse(time(45)), 0.05, 0.5);
    expect(result.eligible).toBe(false);
    expect(result.reason).toContain('warming up');
  });

  it('allows the earliest entry at one minute after persistent observations', () => {
    let state: SignalPersistenceState | undefined;
    for (const seconds of [15, 30, 45, 60]) state = advance(state, seconds);
    const result = evaluateSignalPersistence(state, Date.parse(time(60)), 0.05, 0.5);
  ...
  it('uses distinct 15-second buckets so scheduler jitter cannot produce a permanent 29/30s gate', () => {
    let state: SignalPersistenceState | undefined;
    for (const milliseconds of [15_900, 30_100, 45_200]) state = advanceSignalPersistence(state, {
    ...
    expect(evaluateSignalPersistence(state, Date.parse(time(60)), 0.05, 0.5).eligible).toBe(true);
  });
  ...
  it('requires three observations to span at least thirty seconds', () => {
    let state: SignalPersistenceState | undefined;
    for (const seconds of [60, 65, 70]) state = advance(state, seconds);
    const result = evaluateSignalPersistence(state, Date.parse(time(70)), 0.05, 0.5);
    expect(result.eligible).toBe(false);
  ...
  it('uses median edge rather than one temporary spike', () => {
    let state: SignalPersistenceState | undefined;
    state = advance(state, 30, true, 0.20);
    state = advance(state, 45, true, 0.03);
    state = advance(state, 60, true, 0.03);
    const result = evaluateSignalPersistence(state, Date.parse(time(60)), 0.05, 0.5);
    expect(result.eligible).toBe(false);
