...
import {
  MAKER_RESTRICTION_SENTINEL_VERSION, buildMakerRestrictionSentinelReport,
...
const SNAPSHOT_FILE = path.join(DATA_DIR, 'maker-restriction-sentinels.json');
const JOURNAL_FILE = path.join(DATA_DIR, 'maker-restriction-sentinels.journal.jsonl');
const JOURNAL_COMPACTION_BYTES = 50 * 1024 * 1024;
let operationQueue: Promise<void> = Promise.resolve();
let storeCache: Promise<MakerRestrictionSentinelStore> | undefined;
let storeCacheFingerprint = '';

export interface MakerRestrictionSentinelStore {
  version: 1;
...
export type MakerRestrictionSentinelEvent =
  | { op: 'decision'; value: MakerRestrictionSentinel }
  | { op: 'resolution'; value: MakerRestrictionSentinel };

function emptyStore(startedAt: string): MakerRestrictionSentinelStore {
  return {
    version: 1, sentinelVersion: MAKER_RESTRICTION_SENTINEL_VERSION,
    startedAt, updatedAt: startedAt, sentinels: [],
  };
}

export function replayMakerRestrictionSentinelEvents(
  initial: MakerRestrictionSentinel[], events: MakerRestrictionSentinelEvent[],
...
async function readSnapshot(startedAt: string): Promise<{ store: MakerRestrictionSentinelStore; existed: boolean }> {
  try {
    const parsed = JSON.parse(await readFile(SNAPSHOT_FILE, 'utf8')) as Partial<MakerRestrictionSentinelStore>;
    if (!Array.isArray(parsed.sentinels) || !parsed.startedAt) throw new Error('Maker restriction snapshot is missing required fields.');
    return {
      existed: true,
      store: {
        version: 1,
    ...
    console.error('Maker restriction sentinel snapshot was malformed and has been quarantined:', error);
    return { store: emptyStore(startedAt), existed: false };
  }
}

async function loadStore(startedAt: string): Promise<{ store: MakerRestrictionSentinelStore; existed: boolean }> {
  const loaded = await readSnapshot(startedAt);
  let raw = '';
  try { raw = await readFile(JOURNAL_FILE, 'utf8'); }
  catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error; }
  const events: MakerRestrictionSentinelEvent[] = [];
  const valid: string[] = [];
  for (const line of raw.split('\n')) {
    if (!line) continue;
    try {
      const event = JSON.parse(line) as MakerRestrictionSentinelEvent;
      events.push(event);
      valid.push(line);
    } catch {
      console.error('Maker restriction sentinel journal had a damaged trailing event; preserving its valid prefix.');
      await writeFile(`${JOURNAL_FILE}.corrupt-${Date.now()}`, raw).catch(() => undefined);
      await atomicWrite(JOURNAL_FILE, valid.length ? `${valid.join('\n')}\n` : '');
      break;
    }
  }
  loaded.store.sentinels = replayMakerRestrictionSentinelEvents(loaded.store.sentinels, events);
  return loaded;
}
...
async function readStore(startedAt: string): Promise<{ store: MakerRestrictionSentinelStore; existed: boolean }> {
...
async function cacheStore(store: MakerRestrictionSentinelStore): Promise<void> {
  storeCache = Promise.resolve(store);
  storeCacheFingerprint = await storageFingerprint();
}

async function persistEvents(store: MakerRestrictionSentinelStore, events: MakerRestrictionSentinelEvent[]): Promise<void> {
...
async function ensureStore(observedAt: string): Promise<MakerRestrictionSentinelStore> {
...
export function maintainMakerRestrictionSentinels(observedAt: string): Promise<void> {
 ...
 */
export function recordMakerRestrictionOrder(order: PaperOrder): Promise<void> {
  // Build the small immutable record synchronously; do not clone the growing execution ledger row.
  const sentinel = makerRestrictionSentinelFromOrder(order);
  return serialized(async () => {
    if (!sentinel) return;
...
export function getMakerRestrictionSentinels(): Promise<{ startedAt: string; sentinels: MakerRestrictionSentinel[] }> {
  return serialized(async () => {
