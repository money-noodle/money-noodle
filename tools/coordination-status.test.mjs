...
const ACTIVE_CLAIM = claimIssue({ number: 9, state: "active", checkIn: FAR_FUTURE, label: "work:active" });
...
function writeGhShim(directory, responses) {
  ...
  writeExecutable(directory, "gh", ["#!/bin/sh", SHIM_PATH, 'case "$1" in', ...branches, unexpected, "esac", ""].join("\n"));
}
...
test("a healthy registry prints the plans, claims, and pull requests it can prove", (t) => {
  ...
  assert.match(result.stdout, new RegExp(`branch=test/sample worktree=/fake/worktree check-in=${FAR_FUTURE} \\(current\\)`));
...
test("an unauthenticated GitHub CLI reports coordination unknown instead of an empty board", (t) => {
  const result = runStatus(t, { gh: ghShim({ auth: { status: 1, stderr: "You are not logged into any GitHub hosts\n" } }) });

  assertReportsUnknown(result, { detail: /not authenticated; the shared registry cannot be verified/ });
  assert.doesNotMatch(result.stdout, /## Shared plans/);
});

test("an unreachable issue registry fails loudly and surfaces the underlying error", (t) => {
  const result = runStatus(t, {
    gh: ghShim({ issue: { status: 1, stderr: "error connecting to api.github.com: dial tcp: no such host\n" } }),
  });
...
test("malformed registry output is treated as unknown, not as an empty registry", (t) => {
  const result = runStatus(t, { gh: ghShim({ issue: { stdout: "<html><body>502 Bad Gateway</body></html>\n" } }) });
...
test("a failure after the claim board still marks the whole run unknown", (t) => {
  const result = runStatus(t, { gh: ghShim({ pr: { status: 1, stderr: "gh: connection reset by peer\n" } }) });
...
test("a reachable but empty registry is distinguishable from an unreachable one", (t) => {
  ...
  assert.match(result.stdout, /Registry currently has no open coordinated work\. Confirm this is expected before editing shared scope\./);
});

test("a registry missing coordination labels warns and still exits non-zero", (t) => {
  const partial = COORDINATION_LABELS.filter(({ name }) => name !== "work:blocked" && name !== "work:abandoned");
...
test("an overdue active claim is surfaced for the maintainer and never auto-cleaned", (t) => {
  const overdue = claimIssue({ number: 11, state: "active", checkIn: LONG_PAST, label: "work:active" });
  const blocked = claimIssue({ number: 12, state: "blocked", checkIn: LONG_PAST, label: "work:blocked" });
  const result = runStatus(t, { gh: ghShim({ issue: { stdout: JSON.stringify([overdue, blocked]) } }) });

  assert.equal(result.exitCode, 2);
  assert.match(result.stdout, new RegExp(`check-in=${LONG_PAST} \\(overdue\\)`));
  assert.match(result.stdout, /ASK MAINTAINER: #11 check-in overdue; do not take over or clean automatically/);
...
test("an active claim with no check-in deadline is suspected stale rather than trusted", (t) => {
  const undated = claimIssue({ number: 13, state: "active", checkIn: "unclaimed", label: "work:active" });
  const result = runStatus(t, { gh: ghShim({ issue: { stdout: JSON.stringify([undated]) } }) });

  assert.equal(result.exitCode, 2);
  assert.match(result.stdout, /ASK MAINTAINER: #13 check-in unknown; do not take over or clean automatically/);
});
...
test("an issue holding claim fields but no work:* label is currently invisible to the board", (t) => {
  const claim = claimIssue({ number: 14, state: "active", checkIn: FAR_FUTURE, label: "work:active" });
