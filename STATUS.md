...
- **236 unit tests across 43 files**, passing TypeScript, and passing production build. The authoritative-provider and authenticated-dashboard build was deployed through pause/drain/reconcile/restart; temporary local `AUTH_PASSWORD`/`AUTH_SECRET` values are configured, reconciliation is ready, and live automation was explicitly resumed with zero open orders/reservations.
...
11a. Market identity and per-provider tracking are done. Remaining: split live and paper budgets per provider with percentage market allocations off current provider equity, add per-(provider, market) policy, and keep position/correlation caps global per market. Budget restructuring touches the live reservation path and must be migrated while paused, verified, then resumed.
