...
The follow-up does **not** support calling the recent profitability stable. It supports retaining the model weights, stake, and retry controls. Manual operator promotion deployed binary buy policy v12: UP/YES and DOWN/NO compete on side-specific expected value only when independent selected-side probability is at least 52.5%; the venue-independent probability model remains unchanged.
...
- Binary buy policy v12: at least 52.5% independent selected-side probability, at least 5pp expected edge after fees, at least 50% estimate quality, and an enabled actionable selected-side UP/YES or DOWN/NO entry from 5¢ through 97¢. This is an explicitly operator-promoted, narrow live experiment; v11 results remain separately identifiable in durable issuance evidence.
...
- Manual operator promotion deployed `buy-binary-edge-net5-quality50-owned52.5-price5to97-v12` to live and paper. The 52.5–55% near-miss diagnostic covered only four independent windows (2 wins, +23.5% descriptive mean); it is insufficient as validation. v12 is therefore version-separated in immutable order decisions and forecast history, with maker-only execution, 10¢ configured stake, rate caps, drawdown/lifetime breakers, and all other v11 entry gates unchanged. Do not relax below 52.5% without prospective clustered evidence.
...
- **201 unit tests across 36 files**, passing TypeScript, and passing production build.
