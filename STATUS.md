...
- Unauthenticated visitors on the persistent worker receive only an aggregate read-only paper-budget panel and `GET /api/paper-budget`: simulated starting/available/equity/reserved funds, current paper cap, aggregate outcome counts, P&L, and reset count. The public payload excludes live figures, accounts, provider permissions, order details, symbols, timestamps, and mutations; all control, live-budget, account, provider, history, performance, LLM research, and LLM-provider-management surfaces remain signed-session-only.
- Vercel is explicitly stateless: startup reconciliation, collector timers, execution, and every durable write are disabled; dashboard caches are warm-instance memory only. It returns public market research with a server-side redacted payload and labels its paper ledger unavailable rather than fabricating serverless performance. Signed private routes return a clear persistent-worker-required `503` instead of touching absent files. Production deployment: `https://money-blue-iota.vercel.app`.
