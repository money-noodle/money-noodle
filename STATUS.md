...
- **208 unit tests across 39 files**, passing TypeScript, and passing production build. The authoritative-provider and authenticated-dashboard build was deployed through pause/drain/reconcile/restart; temporary local `AUTH_PASSWORD`/`AUTH_SECRET` values are configured, reconciliation is ready, and live automation was explicitly resumed with zero open orders/reservations.
- Unauthenticated visitors on the persistent worker receive a read-only paper-budget panel/dialog and `GET /api/paper-budget`: simulated starting/available/equity/reserved funds, current paper cap, aggregate outcome counts, P&L, reset count, and the newest 30 grouped sanitized paper execution intents. The public payload excludes all live figures/records, accounts, provider permissions, client/venue identifiers, contracts, decision snapshots, and mutations; full history, control, live-budget, account, provider, performance, LLM research, and LLM-provider-management surfaces remain signed-session-only. The public Policy link exposes immutable model/buy/execution/exit/switch/regime details and history, but never the private provider registry or permissions.
