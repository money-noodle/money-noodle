...
- **213 unit tests across 40 files**, passing TypeScript, and passing production build. The authoritative-provider and authenticated-dashboard build was deployed through pause/drain/reconcile/restart; temporary local `AUTH_PASSWORD`/`AUTH_SECRET` values are configured, reconciliation is ready, and live automation was explicitly resumed with zero open orders/reservations.
- Unauthenticated visitors on the persistent worker see the complete paper-trading side of the system and nothing else. `GET /api/paper-budget` serves the read-only Budget dialog: simulated starting/available/equity/reserved funds, current paper cap, aggregate outcome counts, P&L, reset count, and the newest 30 grouped sanitized paper execution intents. A public paper automation panel reports whether the simulated shadow is active, idle, or bankroll-depleted, its realized P&L, shadow bankroll, open/settled/outcome counts, and its open simulated positions. The positive-edge buy panel is public because the calculation is derived on the client from public predictions and fixed policy thresholds; its per-candidate live-execution and portfolio-readiness badges are omitted because `GET /api/trading/control` stays signed-session-only. `GET /api/paper-performance` serves a public positive-edge track record: allow-listed forecast signal quality (updates, accuracy, cycle-balanced accuracy, Brier, cycle streak, calibration evidence), the newest 12 qualifying calculations without their internal identifiers, and a paper-only executed-money row. The live record is never requested, and the public payload excludes all live figures/records, accounts, provider permissions, client/venue identifiers, contracts, decision snapshots, benchmark/segment/counterfactual/calibration-bin/model-evaluation detail, and mutations; full history, control, live-budget, account, provider, live performance, LLM research, and LLM-provider-management surfaces remain signed-session-only. The public Policy link exposes immutable model/buy/execution/exit/switch/regime details and history, but never the private provider registry or permissions.
