...
import { createTelemetry, type Telemetry } from './create-telemetry.js';
import type { DegradationEvent } from './telemetry-degradation.js';
import { TELEMETRY_LIMITS } from './telemetry-limits.js';
import type { TelemetryTokenSource } from './workload-identity-headers.js';
...
const FORBIDDEN_MARKER = ['FORBIDDEN-MARKER', 'API-PAYLOAD'].join('-');
const syntheticToken = () => ['ya', '29', '.', 'z'.repeat(40)].join('');

const IDENTITY = {
  environment: 'synthetic-production',
  imageDigest: `sha256:${'a'.repeat(64)}`,
  runtimeRevision: 'platform-api-00001-abc',
...
describe('createTelemetry', () => {
  ...
  it('redacts span attributes, names, events and error text before export', async () => {
    ...
    const span = tracer.startSpan(`GET /v1/platform/status?token=${FORBIDDEN_MARKER}`, {
      attributes: {
    ...
    expect(exported?.attributes).toEqual({ 'http.route': '/v1/platform/status' });
  ...
  it('records a bounded request metric and an operational log on the same path', async () => {
    ...
    expect(records[0]?.attributes).toEqual({ 'http.route': '/v1/platform/status' });
...
describe('the production OTLP path', () => {
  ...
  it('serializes a real OTLP payload carrying the route template and no marker', async () => {
    ...
    const span = trace.getTracer('test').startSpan('GET /v1/platform/status', {
      attributes: {
        'http.route': '/v1/platform/status',
        'http.request.body': FORBIDDEN_MARKER,
      },
    ...
    expect(payload.includes('/v1/platform/status')).toBe(true);
