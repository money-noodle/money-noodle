/**
 * Adapter-owned OpenTelemetry composition for the web application.
 *
 * This is the production composition, registered once per Node runtime by
 * `src/instrumentation.ts`. Tests exercise this function, not a parallel
 * handwritten SDK setup: the only thing a test substitutes is the transport (an
 * in-memory exporter or a loopback OTLP sink) and the token source.
 *
 * Nothing here is importable from the browser. `src/presentation` maps DTOs and
 * imports no telemetry; lint and the boundary tests enforce both directions.
 */
...
} from './workload-identity-headers';

/**
 * Span names this application may export verbatim.
 *
 * Next.js emits its own spans automatically once a tracer provider is
 * registered. Their names are route templates by construction, but the
 * allowlist is what guarantees that: a generated name carrying a raw URL is
