...
- CI/CD is mandatory. Only after the [staged delivery transition](docs/operations/delivery.md#current-to-target-activation) is qualified does a qualifying protected-main merge authorize its exact release bundle; agents must not merge unless explicitly asked.
...
- Default authorization to deny, enforce tenant scope at every boundary, keep operational secrets in durable managed storage, and preserve reconstructable audit/accounting records. The selected [M1 catalog](docs/operations/production-control-plane.md#m1-catalog-v2--selected-not-enabled) owns its explicitly bounded sanitized operation-evidence custody; it grants no provider authority and does not weaken tenant/financial audit.
