...
export async function registerNodeRuntime() {
  ...
  const [
    { reconcileLiveExecution, syncCurrentPublicPaperBudgetProjection },
    { startBackgroundCollector },
    { completeExecutionDrain, blockExecutionDrain },
    { startLocalArchiveScheduler },
    { startPeriodicReconciliationScheduler },
  ] = await Promise.all([
    import('./lib/paper-execution'),
    import('./lib/background-collector'),
    import('./lib/execution-drain-state'),
    import('./lib/local-archive-scheduler'),
    import('./lib/periodic-reconciliation'),
  ]);
